...
export async function POST(req: Request) {
	try {
		const getID = await connectionPool.query(
			`SELECT * FROM "User"`
		);
		...
		while (true) {
			const existing = await connectionPool.query(
				`SELECT user_id FROM "User" WHERE user_email = $1 OR username = $2`,
				[user_email, username]
			);
		...
		return NextResponse.json(
			{ message: "User registered successfully", user: result.rows[0], password: password },
			{ status: 201 }
		);
	} catch (err) {
		console.error(err);
